"""
POC live scan component dùng html5-qrcode + st.components.v2.component.
Sẽ promote logic core sang utils/scanner_component.py ở Phase 2.

Source: PLAN_v2.md §Phase 0 Step 2.
...
# HTML template — render iframe với html5-qrcode
_HTML = """
...
# Component factory — call once, cache implicit
...
def live_scanner(key: str | None = None):
    """
    Render live barcode scanner. Returns dict {code, type, ts} when scan
    succeeds, else None. Key bắt buộc unique per location render.
    """
    result = _scanner_component(
        key=key,
        on_scan_change=lambda: None,
    )
    scan = getattr(result, "scan", None) if result else None
    return scan

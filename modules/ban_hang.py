...
# ============================================================
# === POC v2 LIVE SCAN — Phase 0 gate (sẽ xóa ở Phase 5) ===
# ============================================================
def _render_poc_v2_section():
    from utils.auth import is_admin
    if not is_admin():
        return

    with st.expander("🧪 POC v2: Live scan mã vạch (admin)", expanded=False):
        from modules._poc_live_scan import live_scanner
        from utils.db import supabase

        # Toggle pause/resume
        col1, col2 = st.columns([1, 3])
        with col1:
            paused = st.toggle("⏸ Dừng", key="poc_v2_pause", value=False)
        with col2:
            if paused:
                st.caption("Camera tạm dừng — bỏ tick để tiếp tục")
            else:
                st.caption("Camera đang quét — chĩa vào tem mã vạch")

        if paused:
            return

        scan = live_scanner(key="poc_v2_scanner")

        if not scan or not isinstance(scan, dict):
            return

        code = (scan.get("code") or "").strip()
        code_type = scan.get("type", "UNKNOWN")
        scan_ts = scan.get("ts")
        ...
        last_ts = st.session_state.get("_poc_v2_last_ts")
        if last_ts == scan_ts:
            return
        st.session_state["_poc_v2_last_ts"] = scan_ts

        # Lookup ngay
        try:
            res = supabase.table("hang_hoa") \
                .select("ma_hang, ma_vach, ten_hang, loai_sp, is_open_price, gia_ban") \
                .eq("ma_vach", code) \
                .eq("active", True) \
                .limit(2) \
                .execute()
        ...
        st.success(f"✅ Decode: `{code}` (type: **{code_type}**)")

        if not rows:
            st.warning(f"⚠️ Không tìm thấy SP có ma_vach = `{code}`")
        elif len(rows) > 1:
            st.error(f"❌ Duplicate ma_vach: {[r['ma_hang'] for r in rows]}")
        else:
            sp = rows[0]
            st.markdown(f"### 🎯 Tìm thấy: {sp['ten_hang']} (`{sp['ma_hang']}`)")
            st.caption(
                f"Giá: {(sp.get('gia_ban') or 0):,}đ · "
                f"Open-price: {'✅' if sp.get('is_open_price') else '❌'}"
            )
